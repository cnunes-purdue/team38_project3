# Project 3 Final Demonstration: Second Cargo Drop.
# File: Project3_team38_seconddrop.py
...
#           This program instructs the MACRO to follow a line through the course and drop off cargo at point 'B'.
# ---------------------------------------------------
#  Imports
# ---------------------------------------------------
import time
...
try:
    while x != 3:
        ...
        if x == 2:  # Check hall sensor when cargo has not been dropped off, after the first beacon.
            try:
                # Read and store hall sensor value
                value = BP.get_sensor(HALL_PORT)[0]
                if value >= 2100:
...
# ---------------------------------------------------
    while x == 3:  # Check hall sensor when the final beacon has not been passed, after the cargo has been dropped off.
        try:
            # Read and store hall sensor value
            value = BP.get_sensor(HALL_PORT)[0]
            if value >= 2100:
